...
export default class RFB extends EventTargetMixin {
  ...
  _setDesktopName(name) {
    this._fb_name = name;
    this.dispatchEvent(
      new CustomEvent("desktopname", { detail: { name: this._fb_name } })
    );
  }
    ...
    if (reason !== "") {
      this.dispatchEvent(
        new CustomEvent("securityfailure", {
          detail: { status: this._security_status, reason: reason },
        })
      );
      ...
      return this._fail(
        "Security negotiation failed on " + this._security_context
      );
    }
  }
  ...
  _negotiate_xvp_auth() {
    ...
    const xvp_auth_str =
      String.fromCharCode(this._rfb_credentials.username.length) +
  ...
  _negotiate_authentication() {
    switch (this._rfb_auth_scheme) {
      ...
      default:
        return this._fail(
          "Unsupported auth scheme (scheme: " + this._rfb_auth_scheme + ")"
        );
    }
  }
  ...
  _init_msg() {
    switch (this._rfb_init_state) {
      ...
      default:
        return this._fail(
          "Unknown init state (state: " + this._rfb_init_state + ")"
        );
    }
  }
  ...
  _handleVMwareCursor() {
    ...
    if (cursor_type == 0) {
      ...
      for (let pixel = 0; pixel < w * h; pixel++) {
        if (and_mask[pixel] == 0) {
        ...
        } else if ((and_mask[pixel] & PIXEL_MASK) == PIXEL_MASK) {
          ...
          } else if ((xor_mask[pixel] & PIXEL_MASK) == PIXEL_MASK) {
      ...
      Log.Warn(
        "The given cursor type is not supported: " + cursor_type + " given."
      );
      return false;
    }
...
RFB.cursors = {
  ...
  dot: {
    ...
    rgbaPixels: new Uint8Array([
      255, 255, 255, 255, 0, 0, 0, 255, 255, 255, 255, 255, 0, 0, 0, 255, 0, 0,
      0, 0, 0, 0, 0, 255, 255, 255, 255, 255, 0, 0, 0, 255, 255, 255, 255, 255,
    ]),
