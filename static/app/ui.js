...
const PAGE_TITLE = "wt-vnc";
const VM_NAMESPACE = window.VM_NAMESPACE;
const VM_NAME = window.VM_NAME;
const VNC_PATH_PREFIX = window.VNC_PATH_PREFIX;
...
const UI = {
  ...
  setMouseButton(num) {
    ...
    for (let b = 0; b < blist.length; b++) {
      const button = document.getElementById("noVNC_mouse_button" + blist[b]);
  ...
  updateViewOnly() {
    ...
    if (UI.rfb.viewOnly) {
      ...
      document
        ...
        .classList.add("noVNC_hidden");
      document
        .getElementById("noVNC_mouse_button" + UI.rfb.touchButton)
        .classList.add("noVNC_hidden");
    } else {
      document
        .getElementById("noVNC_keyboard_button")
